...
建物3棟・部屋5室。**W Comfort Towers WEST だけ実データが入っている段階。**

| 建物 | 住所・座標 | 部屋 | 写真 |
|---|---|---|---|
| b1 W Comfort Towers WEST | ✅ 東雲1-9-31（座標あり） | p1 39階 16,500万 | 建物1・部屋16 |
| b2 Wコンフォートタワーズ イースト | 未入力 | p2 46階 15,980万 / p3 19階 13,900万 | なし |
| b3 アップルタワー東京キャナルコート | 未入力 | p4 29階 13,980万 / p5 36階 10,980万 | なし |

**まだ全室で未入力なもの**: 登録日（`listedAt`）・価格推移（`priceHistory`）。
このため販売期間・値下げ分析はまだ動かない。
設備タグは p1（39階）にビルトインエアコン・SIC が入っているだけで、
建物側（共用施設・建物構造・建物の設備）は3棟とも空。
