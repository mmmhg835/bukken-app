...
`js/spec.js` が選択肢を定義し、`structureTags` / `facilityTags` / `equipmentTags`（建物）と
`roomEquipmentTags`（部屋）に文字列配列で保存する。自由記述だと表記ゆれ（「オートロック」
「オート ロック」）で比較できなくなるため。選択肢を増やすときは `spec.js` に足すだけでよく、
既存データは触らなくてよい。
